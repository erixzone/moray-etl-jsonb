...
function encodeinternalQuotes(entity) {
   var q = entity.indexOf("\"");
   if (q === -1) {
      return entity;
   }
   var str = null;
   var rem = null;
   str = entity.substr(0,q) + "\\\"";
   rem = entity.substr(q+1,entity.length-1);
   var r = rem.indexOf("\"");
   if (r === -1) {
       return str + rem;
   } else {
       rem = encodeinternalQuotes(rem)
       return str + rem;
   }
}
           ...
           header_tags.keys.forEach(function(elem) {
             ...
             if (nullvalue == true) {
                 ...
                 switch (header_tags.pgtypes[i].type) {
                   ...
                   case 'uuid' :
                       if (arraycount == 0) {
                        // Removing this Joyent patch for string 'default' in uuid field
